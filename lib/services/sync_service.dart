...
class SyncService {
  ...
  Future<void> syncOfflineData() async {
    ...
    if (unsyncedReports.isNotEmpty) {
      debugPrint("==== SYNCING ${unsyncedReports.length} REPORTS TO FIREBASE ====");
      // TODO: FirebaseFirestore.instance.collection('reports').add(...)
      
      // Mark as synced locally
      for (var report in unsyncedReports) {
        await db.update('reports', {'is_synced': 1}, where: 'id = ?', whereArgs: [report['id']]);
      }
    }
    ...
    if (unsyncedAccounts.isNotEmpty) {
      debugPrint("==== SYNCING ${unsyncedAccounts.length} ACCOUNTS TO FIREBASE ====");
      // TODO: FirebaseFirestore.instance.collection('users').doc(amomimusId).set(...)
      
      for (var acc in unsyncedAccounts) {
        await db.update('accounts', {'is_synced': 1}, where: 'id = ?', whereArgs: [acc['id']]);
      }
    }
    ...
    if (unsyncedBlocks.isNotEmpty) {
      debugPrint("==== SYNCING ${unsyncedBlocks.length} BLOCKS TO FIREBASE ====");
      // TODO: Push blocks
      for (var b in unsyncedBlocks) {
        await db.update('blocks', {'is_synced': 1}, where: 'id = ?', whereArgs: [b['id']]);
      }
    }
    ...
    if (unsyncedItems.isNotEmpty) {
      ...
      for (var item in unsyncedItems) {
        await db.update('user_items', {'is_synced': 1}, where: 'id = ?', whereArgs: [item['id']]);
      }
    }
  }
  ...
  Future<void> receiveAdminIndicatorUpdate(String amomimusId, String newIndicator, int newPoints) async {
    final db = await SqliteService.instance.database;
    await db.update(
      'accounts',
      {'indicator': newIndicator, 'benevolentPoints': newPoints, 'is_synced': 1},
      where: 'amomimusId = ?',
